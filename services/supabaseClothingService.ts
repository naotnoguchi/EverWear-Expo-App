...
export const getClothingItems = async (): Promise<AppClothingItem[]> => {
  ...
  for (const item of items) {
    ...
    const { data: washHistory, error: washError } = await supabase
      .from('wash_history')
      .select('*')
      .eq('clothing_item_id', item.id);
...
export const deleteWearRecord = async (wearId: string): Promise<void> => {
  ...
  const { data: wearRecord, error: getError } = await supabase
    .from('wear_history')
  ...
  const { data: latestWear, error: latestError } = await supabase
    .from('wear_history')
    .select('date')
...
export const deleteWashRecord = async (washId: string): Promise<void> => {
  ...
  const { data: washRecord, error: getError } = await supabase
    .from('wash_history')
    .select('clothing_item_id')
    .eq('id', washId)
    .single();
